...
# update mirror
sudo sed -i -E '/ubuntu/s|https?://[^ ]+|https://mirror.netzwerge.de/ubuntu|g' /etc/apt/sources.list.d/official-package-repositories.list
sudo sed -i -E '/mint/s|https?://[^ ]+|https://mirror.netzwerge.de/linuxmint/packages|g' /etc/apt/sources.list.d/official-package-repositories.list
...
sudo -E bash /usr/local/sbin/linux_setups_debian_install.sh
